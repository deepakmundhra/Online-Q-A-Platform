	...
	<body>
			...
			<div id="contentsm">
				<p align="center"><br>
					...
					<table>
						<tr></tr>
						<tr><td>";
							$sqlll = "SELECT question_id,question,users_id,category,datetimes FROM questions WHERE question_id='{$qid}'";
							$res=$db->query($sqlll) or die($db->error);
						    $rws=$res->fetch_array();
						    $su="SELECT email FROM user_details WHERE users_id='{$rws[2]}'";
  							$res=$db->query($su) or die($db->error);
  							$rw=$res->fetch_array()or die($db->error);
  							echo "<h2>Q) ".$rws[1]."</h2></td></tr><tr><td><br>";
				 			echo "<b>ID:</b>".$rws[0]." <b>Asked by:</b>".$rw[0]."<br><b>Category:</b>".$rws[3]." <b>Posted:</b>".$rws[4]."<br><br>";
				 			echo"<tr><td><textarea name='a' placeholder='Write Answer' rows='5' cols='40'></textarea></td></tr>
					...
					{
						$answ=$_REQUEST['a'];
						$sql = "INSERT INTO answers VALUES('','{$answ}','{$rws[0]}','{$rws[2]}','','{$rws[3]}',NOW())";
						$db->query($sql) or die($db->error);
						if($db->affected_rows==1)
						{
						echo"<h2>Successfully Posted!!</h2>
					<br><a href='answerquestion.php'>Continue</a>";
					}
					else
						echo "Error";
				}
		...
		<div id="rightm">
			<table align="center" width="90%">
						...
						<tr><td align="center">
						<form method="POST" >
						<input type="submit" value="Ask a question" id="catbut" style="height:20px; width:150px;" name="askquestion"<?php if(empty($_SESSION['value'])) echo "onclick='error()'";?>/><form></td></tr>
						<form method="POST">
						<tr><td align="center"><input type="submit" id="catbut" style="height:20px; width:150px;" value="Answer a question" name="answerquestion"<?php if(empty($_SESSION['value'])) echo "onclick='error()'";?>/></td></tr>
						</form>
